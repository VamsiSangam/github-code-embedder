...
Plugin URI: https://vamsisangam.github.io/
Description: A WordPress plugin to embed source code hosted on GitHub with syntax highlighting.
Version: 1.0
Author: Vamsi Sangam
Author URI: https://github.com/VamsiSangam/github-code-embedder
License: MIT
*/
if (is_admin()) {
    /* Call the html code */
...
function github_code_embedder_wp_footer() {
    ...
    <script src="<?= plugins_url() ?>/github-code-embedder/public/js/prism.js"></script>
    <script>
...
function github_code_embedder_wp_head() {
    wp_enqueue_style('prismjs.css', plugins_url() . '/github-code-embedder/public/css/prism.css');
}

add_action('wp_head', 'github_code_embedder_wp_head');

function gh_code_func($atts = [], $content = null) {
    $o = '<!-- Generated by GitHub code embedder -->';
    $o .= '<pre class="dynamic-prism language-' . $atts['lang'] . ' line-numbers">';
    $o .= '<code data-github-raw="' . $atts['url'] . '"></code>';
    $o .= '</pre>';
